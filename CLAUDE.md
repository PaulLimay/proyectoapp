...
**Punto de entrada / navegación**: `MainActivity` envuelve todo en `LigorTravelTheme` y renderiza `LigorNavGraph` (`ui/navigation/LigorNavGraph.kt`). Hay cuatro rutas, definidas en `object Rutas`: `SPLASH` (destino inicial), `AUTH`, `HOME` y `PERFIL`. El flujo es `Splash → Auth → Home → Perfil`, con `Perfil → Auth` al cerrar sesión. `SplashScreen` (`ui/splash/SplashScreen.kt`) muestra "BIENVENIDOS A LIGOR TRAVEL" durante 2 segundos (`LaunchedEffect` + `delay`) y navega sola a `AUTH`. Todas las transiciones de avance (`Splash → Auth`, `Auth → Home`, `Perfil → Auth`) sacan la ruta anterior del back stack (`popUpTo(..., inclusive = true)`), así que no hay navegación hacia atrás entre splash/estado no autenticado/estado autenticado; la única transición que sí deja rastro en el back stack es `Home → Perfil` (navegación normal, sin `popUpTo`), para poder volver de Perfil a Home con el botón atrás.
